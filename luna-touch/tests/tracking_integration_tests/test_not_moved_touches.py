...
def prev_touches():
    ...
    prev_touches[0] = Touch((50,50),   20)
    prev_touches[1] = Touch((100,100), 20)
    prev_touches[2] = Touch((200,200), 20)
...
@pytest.fixture
def final_touches():
    final_touches = prev_touches=OrderedDict()

    final_touches[0] = Touch((50,50),   20)
    final_touches[1] = Touch((100,100), 20)
    final_touches[2] = Touch((200,200), 20)
    
 
    final_touches[0].state = _const.TOUCH_STATE_STILL
    final_touches[1].state = _const.TOUCH_STATE_STILL
    final_touches[2].state = _const.TOUCH_STATE_STILL
...
def test_touch_tracking_for_not_moved_touches(prev_touches, curr_touches, final_touches):
